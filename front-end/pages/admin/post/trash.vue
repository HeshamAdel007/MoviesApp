      ...
      table.w-full.text-md.bg-white.shadow-md.rounded.mb-4
        tbody
          tr.border-b
            ...
            th.text-left.p-3.px-5 Views
            th.text-left.p-3.px-5 Like
            th Actions
          <!-- Spinner -->
          .spin
            spinner(v-if="show")
          tr.border-b.bg-gray-100(v-for="(post, index) in posts" :key="index" class='hover:bg-orange-100')
            td.p-3.px-5
            ...
            td.p-3.px-5
              .bg-transparent {{ post.view_count }}
            td.p-3.px-5
              .bg-transparent {{ post.post_like }}
            td.p-3.px-5.flex.justify-end
              button.mr-3.text-sm.bg-blue-500.text-white.py-1.px-2.rounded(@click="restore(post.id)" type='button' class='hover:bg-blue-700 focus:outline-none focus:shadow-outline') restore
              button.text-sm.bg-red-500.text-white.py-1.px-2.rounded(@click="hardDelete(post.id)" class='hover:bg-red-700 focus:outline-none focus:shadow-outline') delete
</template>
...
export default {
  ...
  methods: {
    ...mapActions({
      allPost: "post/postTrash",
      backPost: "post/restorePost",
      postDelete: "post/hardDelete",
    }),
    restore(id) {
      // Icon Image Snotify
      ...
      const postId = id;
      const userToken = localStorage.getItem("token");
      this.backPost({ id: postId, token: userToken })
        .then(response => {
          this.$snotify.success(response.data.message, "Restore Post", {
            icon: iconSuccess,
          });
        ...
        .catch(error => {
          ...
          if (error.response.status === 500) {
            this.$snotify.error("Can't Restore Post Please Try Again Later", "Restore Post", {
              icon: iconError,
            });
          }
        });
    ...
    hardDelete(id) {
      ...
      this.postDelete({ id: postId, token: userToken })
        ...
        .catch(error => {
          if (error.response.status === 500) {
            this.$snotify.error("Can't Delete Post Please Try Again Later", "Delete", {
              icon: iconError,
            });
          }
        });
